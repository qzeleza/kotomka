...
для роутеров Kennetic на таких языках программирования, как Си, С++ и на семействе языков Bash.
...
1. Быстрое развертывание среды разработки для роутеров Keenetic в Entware - от 30 минут (для полной сборки образа).
...
3. На текущий момент собрать можно только для одной архитектуры процессора и одного приложения (в планах добавить возможность автосборки под несколько архитектур проц. и под несколько приложений в пакете.). 
...
7. Тесты основаны на пакете bats и предназначены прежде всего для проверки запуска приложения с аргументами, для любого языка разработки.
...
```
   /apps/kotomka/                          - папка настоящего проекта (имя папки необходимо изменить на имя 
          ...
          ├──LICENCE.ru                    - файл с описанием лицензии на русском по которой распространяется данный проект.
          ├──LICENCE.en                    - файл с описанием лицензии на англиском по которой распространяется данный проект.
          ├──CONTRIBUTIONS                 - файл содержит ответы на вопросы 'Как помочь проекту?' и 'Как составить тикет?'.
...
------------------------------------------- Директории c исходным кодом  ----------------------------------------------------    
          ...
          |                                  Данный файл подлежит редактированию. Значния этих переменных необходимо
          |                                  редактировать в файле ./build/CONFIG
          |
          └──/tests/                       - папка, в которой находятся тесты для запуска на удаленной машине.
               |                                             
               ├──tests.run                - основной файл запуска тестов, который запускает все тесты из паки modules. 
               └──funcs.lib                - файл содержит фукнции для работы файла tests.run.    
                     └──/modules/          - папка, в котрой находятся все тесты проекта распределенные по отдельным файлам. 
                           ├──hello.bat    - тест для проверки программы-примера выводящей в консоль роутера 'Здравствуй Мир!'.  
                           └──library      - файл библиотечных функций необходимых для запуска тестов.   
```
...
3. После сборки образа, контейнера и самого пакета, автоматически произойдет копирование собранного пакета на Ваше устройство и запрос на запуск тестов, с целью проверки работы.
...
6. Запускаем сборку пакета командой `./kotomka make`
   ...
   - Возможна альтернативная сборка пакета, путем входа в контейнер и запуска скрипта по команде `cd /apps/<имя_проекта>/build && ./scripts/package.app`
...
| ./kotomka \<**cmd**> **debug** |       **-vb**        | вывод отладочной информации при сборке  |
| ./kotomka **init**             |       **-in**        | возвращение к первоначальному состоянию |
| ./kotomka **test**             |       **-ts**        | запуск тестов внутри контейнера         |
...
test        - запускается выполнение тестов без пакета сборки (предполагается, что сам пакет уже установлен 
              на устройство). Тесты должны быть написаны заранее и располагаются в папке ./tests.              
              Подробная документация по написанию тестов находится здесь: 
...
преобразуется в файл **Makefile** для соответствующего языка разработки и копируется в корень папки **/apps/entware/packages/utils/<имя_пакета>**. 
...
3. Для дополнительной информации по вопросам сборки пакетов на Entware для роутеров Keenetic обратитесь к источнику по этой [ссылке](https://openwrt.org/docs/guide-developer/helloworld/chapter1).
---
...
```
   /apps/
      ...
      └──/entware/                                 - папка, в котрой находятся файлы сборки пакета entware для Keenetic. 
          ...
          |      |   |       └──/files             - папка, в которой находятся все учавствующие в сборке файлы проекта, 
