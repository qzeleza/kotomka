...
connect_when_run(){
    ...
    echo "${_user}::Производим подключение к контейнеру id = ${docker_id}."
...
connect_when_not_mounted(){
    ...
    echo "${PREF}ЗАХОДИМ ВНУТРЬ КОНТЕЙНЕРА ${APP_NAME}"
...
#    docker container prune -f
    ...
    #	если не создан образ, то запускаем сборку образа
        ...
        sedi "s|.*-.*:.*{APPS_ROOT}.*{APP_NAME}|        - ${dev_path//\//\\/}:\${APPS_ROOT}\/\${APP_NAME}|g; \
            s|dockerfile:.*$|dockerfile: ${DOCKER_FILE}|" "${DOCKER_COMP_FILE}"
    ...
    if [ "${?}" = 0 ]; then
        ...
        echo "${PREF}Запускаем сборку пакета в самом контейнере..."
        docker exec -w "${APPS_ROOT}/${APP_NAME}/build" --user "${USER}:${GROUP}" -it "${APP_NAME}" /bin/bash "${script_to_run}"
        rm -f "${ENV_FILE}"
    else
        show_line; echo "${PREF}Docker-образ собран с ошибками!"
        exit 1
    fi
    show_line
}
