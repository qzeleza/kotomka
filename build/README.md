# [КАТОМКА]: Руководство по быстрому разврертыванию среды разработки Entware для роутеров Keenetic для сборки отдельных пакетов на языках С, С++, семейства Bash в Docker-контейнере.

```
Данный пакет служит прежде всего для тех разработчиков, которые хотят быстро развернуть сборку своего пакета 
для роутеров Kennetic на таких языках программирования, как **С**, **С++**, семейство языков **Bash**.

Установка данного пакета на хост-машину позволит быстро собрать Вам необходимый docker-образ со средой разработки 
под Entware с программой-примером, которая, при ее запуске, выведет на консоль "Здравствуй Мир!".

После чего Вы уже самостоятельно можете под свои нужды модифицировать данный пример и расширять его, по мере 
необходимости, используя свою IDE и запуская сборку внутри запущенного контейнера с автоматическим удалением старого 
пакета и установкой новой версии пакета на удаленное устройство.
```
---
...
## Основные принципы сборки среды разработки, используемые в **Катомке**.
...
## Структура файлов проекта **Катомка**.
```
   /apps/katomka/                          - папка настоящего проекта (имя папки необходимо изменить на имя 
          ...
          ├──vesrion                       - файл с данными о текущей версии разработаываемого приложения.
          ├──katomka                       - ОСНОВНОЙ файл запуска сборки образа, контейнера и самого приложения в нем 
          ...
          |    |    ├──Makefile.app        - скрипт, который запускается для создания 
          ...
          |         ├──/compile/           - папка, которая содержит файлы манифеста Makefile и скрипты postinst и postrm, 
          |         |                        используемые для сборки пакета. 
          ...
          |                                  (появляются только после запуска ./build/katomka). Здесь их можно редактировать в своей IDE.
          └──/make/                        - папка, в котрой находятся файлы манифеста Makefile и скрипты postinst и postrm, 
                                             используемые для сборки пакета (появляются только после запуска ./build/katomka). 
...
```
mkdir ./apps 
git clone https://github.com/qzeleza/katomka.git
...
2. Запускаем сборку docker-образа/контейнера в автоматическом режиме посредством файла `./katomka build`
   - можно воспользоваться командой `./katomka make` для последующей сборки самого пакета
   - в случае необходимости, можно сразу запустить сборку в режиме отладки командой `./katomka make debug`
...
6. Запускаем сборку пакета командой **./apps/<имя_пакета>/build/katomka make**
...
### Описание параметров запуска утилиты **build/katomka**
| Команда                        | Сокращение | Краткое описание                        |
|--------------------------------|:----------:|:----------------------------------------|
| ./katomka **build**            |  **-bl**   | собираем docker-образ                   |
| ./katomka **rebuild**          |  **-rb**   | пересобираем docker-образ               |
| ./katomka **make**             |  **-mk**   | сборка разрабатываемого пакета          |
| ./katomka **copy**             |  **-cp**   | копирование пакета на роутер            |
| ./katomka **term**             |  **-tm**   | подключение к контейнеру под **$USER**  |
| ./katomka **root**             |  **-rt**   | подключение к контейнеру под **root**   |
| ./katomka \<**cmd**> **debug** |  **-db**   | вывод отладочной информации при сборке  |
| ./katomka **data_init**        |     -      | возвращение к первоначальному состоянию |

```
build       - сборка образа и последующий запуск сборки пакета. Для последующей сборки пакета необходимо 
              исполнить, либо команду 'katomka make' на хостовой машине, либо, зайдя в собранный контейнер 
              при помощи команды 'katomka term', выполнить скрипт /apps/<имя_проекта>/build/scripts/package.app
...
term        - подключение к docker-контейнеру без исполнения скриптов под текущим '$USER'. Обычный режим 
              ...
              пакета с хостовой машины посредством команды 'katomka make' будет не возможно отвечать на запросы 
...
data_init   - полное удаление исходных файлов в папках <имя_пакета>/{code,make} и последующая иницилизация 
